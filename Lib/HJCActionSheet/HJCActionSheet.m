...
- (instancetype)initWithDelegate:(id<HJCActionSheetDelegate>)delegate CancelTitle:(NSString *)cancelTitle OtherTitles:(NSString *)otherTitles, ...
{
    ...
    self.titleBtnAry = [NSMutableArray array];
    ...
    [btn setTitleColor:mRGBToColor(0xDF0526) forState:UIControlStateNormal];
    btn.titleLabel.font = HeitiLight(18);
    btn.tag = 0;
...
- (void)setupBtnWithTitle:(NSString *)title{
    ...
    if(_tag-2<self.colorsArray.count)
        [btn setTitleColor:mRGBToColor([[self.colorsArray objectAtIndex:_tag-2] integerValue]) forState:UIControlStateNormal];
    else
    ...
    [btn addTarget:self action:@selector(sheetBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [self.sheetView addSubview:btn];
    [self.titleBtnAry addObject:btn];
