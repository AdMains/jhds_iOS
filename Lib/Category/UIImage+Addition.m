...
- (UIImage *)imageWithCsdnText
{
    ...
    CGRect rect = CGRectMake(self.size.width/2-12*7, self.size.height/2-60, self.size.width, self.size.height);
    [@"C S D N" drawInRect:CGRectIntegral(rect) withAttributes:@{NSFontAttributeName: [UIFont  systemFontOfSize:self.size.width<=320?40:40*(self.size.width/320.0)], NSForegroundColorAttributeName :mRGBToColor(0xc4c4c4)}];
    
    CGRect rect2 = CGRectMake(self.size.width/2-12*7, self.size.height/2, self.size.width, self.size.height);
    [@"全球最大中文IT社区" drawInRect:CGRectIntegral(rect2) withAttributes:@{NSFontAttributeName: [UIFont  systemFontOfSize:self.size.width<=320?18:18*(self.size.width/320.0)], NSForegroundColorAttributeName :mRGBToColor(0xc4c4c4)}];
    UIImage *newImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return newImage;
}
